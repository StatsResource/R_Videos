...
(2)
vi) Fit an AR(1), AR(3) and ARMA(1,1) model to the time series.
(3)
...
Page 2 of 15
Solution 1:
i) set.seed(100) observations <- arima.sim(list(order = c(1,1,1), ar = 0.9, ma = 0.2), n = 500) plot(observations, main = "Line chart of time series observation")
[3]
ii)
...
Page 3 of 15
[2]
iv) x = 1:501 leastsquarefit <- lm(observations~x) leastsquarefit$coefficients (Intercept) x 10.0453231 -0.3941461 plot(observations, main = "Line chart of time series observation") abline(leastsquarefit)
[3]
IAI CS2B-1123
Page 4 of 15
...
[2]
vi) fit1 = arima(observations, order= c(1,0,0)) fit1 Call: arima(x = observations, order = c(1, 0, 0))
IAI CS2B-1123
Page 5 of 15
Coefficients: ar1 intercept 0.9997 -89.3179 s.e. 0.0004 85.5509 sigma^2 estimated as 5.091: log likelihood = -1122.25, aic = 2250.5
fit2 = arima(observations, order= c(3,0,0)) fit2 Call: arima(x = observations, order = c(3, 0, 0)) Coefficients: ar1 ar2 ar3 intercept 2.0064 -1.1350 0.1278 -89.0734 s.e. 0.0443 0.0864 0.0444 46.5760 sigma^2 estimated as 1.01: log likelihood = -718.61, aic = 1447.22
...
[3]
vii) fit1$coef[1] - qnorm(0.975)*sqrt(fit1$var.coef[1,1]) ar1 0.998827 fit1$coef[1] + qnorm(0.975)*sqrt(fit1$var.coef[1,1]) ar1 1.000529
The confidence interval is ( 0.998827 , 1.000529 )
[2]
viii)
The AIC is lowest for AR(3) among the models above and hence is the best fit among the above models. predict(fit2, n.ahead = 10) $pred
IAI CS2B-1123
Page 6 of 15
